...
function buildResult({
  ...
  frameRateDelta: number;
  sceneChangeRate: number;
  identityScore: number;
  identityFrameScores: number[];
  identitySampleTimesSec: number[];
  motionScore: number;
  motionSimilarityScore: number;
  motionSampleTimesSec: number[];
  freezeRisk: number;
  sourceMotionEnergy: number;
  outputMotionEnergy: number;
...
function sampleTimes(durationSec: number, count = 5): number[] {
...
async function extractFrameAtTime(videoPath: string, timeSec: number, outputPath: string): Promise<void> {
...
async function computeSsimScore(inputA: string, inputB: string): Promise<number> {
...
async function probeDecode(videoPath: string): Promise<{ success: boolean }> {
  try {
    const { stderr } = await execFileAsync("ffmpeg", [
      "-v",
      "error",
...
async function computeFrameBrightness(imagePath: string): Promise<number> {
...
async function extractFrames(videoPath: string, timesSec: number[], frameDir: string, prefix: string): Promise<string[]> {
  const frames: string[] = [];
...
async function computeAdjacentMotionEnergy(framePaths: string[]): Promise<number> {
...
async function computeBrightnessRatio(framePaths: string[]): Promise<number> {
...
export async function evaluateRun({ sourceVideoPath, referenceImagePath, outputVideoPath }: { sourceVideoPath: string; referenceImagePath: string; outputVideoPath: string }) {
  ...
  try {
    ...
    const motionFrameScores: number[] = [];
